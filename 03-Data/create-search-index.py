...
def batch_upload_documents(search_client, docs, batch_size=1000):
    for i in range(0, len(docs), batch_size):
        batch = docs[i:i+batch_size]
        print(f"Uploading {len(batch)} documents to index {AZURE_SEARCH_INDEX_NAME}")
...
def initialize(search_index_client: SearchIndexClient):
    ...
    for i in range(num_batches):
        ...
        print(f"Processed batch {i + 1}/{num_batches}. Remaining: {num_batches - (i + 1)}")
...
def load_variables():
    ...
    global DATA_DIR
    DATA_DIR = os.path.join(os.curdir, script_dir + "/../data/azuresearch/well-architected")
